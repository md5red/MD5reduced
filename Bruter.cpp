...
void CBruter::Loop_4char(const int threadIdx)
{
	...
	// This is our (padded) plain package...
	// Doing the padding in advance saves
	// a few hundredth of a second.
	unsigned char plainPack[PACKAGE_SIZE][8];
	for (int i = 0; i < PACKAGE_SIZE; i++) {
		// Null it.
		memset(plainPack[i], 0, 8);

		// Terminate it.
		plainPack[i][length] = (char)0x80;
	}

	// ... and our current plain with the message
	...
	// A few variables:
	// index counts to PACKAGE_SIZE.
	// ret is the index of the cracked hash.
	int index = 0, ret = 0;
		...
		for (int i1 = 0; i1 < myCharset->GetLen(); i1++) {
			...
			for (int i2 = 0; i2 < myCharset->GetLen(); i2++) {
				...
				for (int i3 = 0; i3 < myCharset->GetLen(); i3++) {
					...
					// Copy the plain to our wordPack.
					memcpy(plainPack[index], plain, length);

					// We got 12?
					if (++index >= PACKAGE_SIZE) {
						ret = MD5Calculate(plainPack, length, threadIdx);
						if (ret != -1) {
							// Copy to result.
							memcpy(m_cResult, plainPack[ret], length);
	...
	// Any left overs after the main loops?
	if (index > 0) {
		ret = MD5Calculate(plainPack, length, threadIdx);
		if (ret != -1) {
			// Copy to result.
...
void CBruter::Loop_8char(const int threadIdx)
{
	...
	// Doing the padding in advance saves
	// a few hundredth of a second.
	unsigned char plainPack[PACKAGE_SIZE][12];
	for (int i = 0; i < PACKAGE_SIZE; i++) {
		// Null it.
		memset(plainPack[i], 0, 12);
						...
						for (int i1 = 0; i1 < myCharset->GetLen(); i1++) {
							...
							for (int i2 = 0; i2 < myCharset->GetLen(); i2++) {
								...
								for (int i3 = 0; i3 < myCharset->GetLen(); i3++) {
									...
									// We got 12?
									if (++index >= PACKAGE_SIZE) {
										ret = MD5Calculate_8plus(plainPack, length, threadIdx);
										if (ret != -1) {
											// Copy to result.
											memcpy(m_cResult, plainPack[ret], length);
